...
Run: python -m source_distiller.evaluate
...
def index_sources(src: Path, out: Path) -> dict:
    run([sys.executable, "-m", "source_distiller.cli", "index", str(src), "--out", str(out)])
    return json.loads(out.read_text(encoding="utf-8"))


def search(index: Path, query: str, top_k: int = 5) -> list[dict]:
    proc = run([
        sys.executable, "-m", "source_distiller.cli",
        "search", "--index", str(index), "--query", query,
...
def evaluate_quote(index: Path, index_data: dict) -> tuple[int, int, list[str]]:
    ...
    proc = run([sys.executable, "-m", "source_distiller.cli", "quote", "--index", str(index), "--cite", cite])
...
def evaluate_audit(index: Path, index_data: dict, base: Path) -> tuple[int, int, list[str]]:
    ...
    good_proc = run([
        sys.executable, "-m", "source_distiller.cli",
        "audit", "--index", str(index), "--answer", str(good),
    ], check=False)
    ...
    bad.write_text(
        ...
        "This long paragraph deliberately has no citation even though it states a "
        "source-dependent claim about policy behavior and should therefore be flagged "
        "by the audit command.\n",
        encoding="utf-8",
    )
    bad_proc = run([
        sys.executable, "-m", "source_distiller.cli",
        "audit", "--index", str(index), "--answer", str(bad),
    ], check=False)
    bad_ok = (
...
def evaluate_semantic_gap(index: Path, index_data: dict, base: Path) -> tuple[int, int, list[str]]:
    ...
    proc = run([
        sys.executable, "-m", "source_distiller.cli",
        "audit", "--index", str(index), "--answer", str(false_answer),
    ], check=False)
...
def evaluate_conflicts(index: Path) -> tuple[int, int, list[str]]:
    """The fixture set has policy v1 (30 days, 10 MB) vs v2 (90 days, 50 MB) and
    engineering notes (10 MB). Conflicts command should find at least one pair."""
    proc = run([
        sys.executable, "-m", "source_distiller.cli",
        "conflicts", "--index", str(index), "--json",
    ])
    data = json.loads(proc.stdout)
    found = len(data) > 0
    return int(found), 1, [
        f"{'PASS' if found else 'FAIL'} conflicts detects cross-source disagreements ({len(data)} found)"
    ]


def evaluate_report(index: Path) -> tuple[int, int, list[str]]:
    proc = run([
        sys.executable, "-m", "source_distiller.cli",
        "report", "--index", str(index), "--query", "audit log retention",
    ])
    has_map = "## Source Map" in proc.stdout
    has_evidence = "## Evidence for:" in proc.stdout
    ok = has_map and has_evidence
    return int(ok), 1, [
        f"{'PASS' if ok else 'FAIL'} report generates source map + evidence"
    ]
...
def main() -> int:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        c_pass, c_total, c_details = evaluate_conflicts(index_path)
        rp_pass, rp_total, rp_details = evaluate_report(index_path)

        all_details = r_details + q_details + a_details + s_details + c_details + rp_details

        mechanical_pass = r_pass + q_pass + a_pass + c_pass + rp_pass
        mechanical_total = r_total + q_total + a_total + c_total + rp_total
        ...
        print(f"conflicts_score:    {c_pass}/{c_total} = {pct(c_pass, c_total)}%")
        print(f"report_score:       {rp_pass}/{rp_total} = {pct(rp_pass, rp_total)}%")
        print(f"semantic_score:     {s_pass}/{s_total} = {pct(s_pass, s_total)}%")
        print(f"mechanical_total:   {mechanical_pass}/{mechanical_total} = {pct(mechanical_pass, mechanical_total)}%")
        print(f"overall_score:      {full_pass}/{full_total} = {pct(full_pass, full_total)}%")
        print()

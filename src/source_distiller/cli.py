...
def _number_conflicts(text_a: str, text_b: str) -> list[tuple[str, str]]:
    ...
    pat = re.compile(r"(\b[a-z]{3,})\s+(?:of\s+|is\s+|are\s+|=\s*)?(\d+(?:\.\d+)?(?:\s?%)?)", re.I)
    nums_a: dict[str, set[str]] = defaultdict(set)
    nums_b: dict[str, set[str]] = defaultdict(set)
    for m in pat.finditer(text_a):
        nums_a[m.group(1).lower()].add(m.group(2))
    for m in pat.finditer(text_b):
        nums_b[m.group(1).lower()].add(m.group(2))
    conflicts = []
    ...
    for i, ca in enumerate(chunks):
        for j in range(i + 1, len(chunks)):
            ...
            if ca["source_id"] == cb["source_id"]:
                continue
            sim = _jaccard(topic_cache[ca["id"]], topic_cache[cb["id"]])
            if sim < min_similarity:
                continue
            neg = _negation_score(ca["text"]) + _negation_score(cb["text"])
            num_conflicts = _number_conflicts(ca["text"], cb["text"])
            conflict_score = sim * 0.4 + min(neg, 5) * 0.1 + len(num_conflicts) * 0.3
